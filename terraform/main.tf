...
module "web-server" {
  ...
  ami                     = data.aws_ami.ansible-nginx.id  #  Using AMI from Packer
  instance_type           = "t2.micro"  #  Default instance type
  key_name                = "aws-4640"  #  Using key imported with import script
  vpc_security_group_ids  = [aws_security_group.web.id]  #  Subnet ID from main
  subnet_id               = aws_subnet.web.id
}
